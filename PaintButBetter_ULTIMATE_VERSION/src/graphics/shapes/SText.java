...
public class SText extends Shape{
	...
	public void changeSize(int x, int y, boolean top, boolean e) {
		...
		if (top && size-x-y > 1) {
			size -= x+y;
			font.setFont(size, font.getFont().getName(), font.getFont().getStyle());
			font.setSize(size);
			this.loc.x += x+y;
			this.loc.y += x+y;
		}
	...
	public SText copy() {
        ...
        new_text.addAttributes(new FontAttributes(font.getFont().getName(), font.getFont().getStyle(), font.getFont().getSize()));
        FontAttributes new_font = (FontAttributes) new_text.getAttributes("font");
        new_font.setFont(font.getFontSize(), font.getFont().getName(), font.getFont().getStyle());
